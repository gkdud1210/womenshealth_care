 ...
 * Fuses cycle phase, EEG, thermal, iris, and bio-signal data
...
export function askLudia(
  ...
  const stressHigh   = data.eeg.stressIndex >= 65
  const stressMid    = data.eeg.stressIndex >= 45
  ...
  const ansSym       = data.eeg.ansBalance < 45
    ...
    return {
      text: `현재 D+${cycleDay}일 ${phaseName}이에요. 다음 생리 예정은 약 ${daysLeft}일 후입니다. ${
        phase === 'luteal' && cycleDay >= 21
          ? `PMS 구간에 접어들었어요. EEG 스트레스 ${data.eeg.stressIndex}점 ${stressHigh ? '— 베타파가 높아 불안·예민함이 올 수 있으니 지금부터 마그네슘 보충을 시작하면 좋아요' : '— 현재는 관리 가능한 수준이에요'}. 카페인을 줄이고 탄수화물보다 단백질 위주 식사를 권장해요.`
          : phase === 'ovulation'
  ...
  // ── EEG / STRESS / BRAINWAVE ───────────────────────────────────
  if (hit(q, ['스트레스', '뇌파', 'eeg', '긴장', '불안', '집중', '알파', '베타', '뇌', '멘탈', '머리'])) {
    return {
      text: `뇌파 분석 결과에요. 스트레스 지수 ${data.eeg.stressIndex}/100 ${stressHigh ? '— 상당히 높은 수준이에요' : stressMid ? '— 보통 수준이에요' : '— 안정적이에요'}. 알파파(이완) ${data.eeg.alphaRatio}% vs 베타파(각성) ${data.eeg.betaRatio}%로 현재 ${data.eeg.betaRatio > data.eeg.alphaRatio ? '각성·긴장 상태' : '이완 상태'}예요. 자율신경 균형은 부교감 ${data.eeg.ansBalance}% — ${ansSym ? '교감신경이 우세해서 몸이 계속 긴장 모드에 있어요' : '균형이 잘 맞아요'}. ${
        stressHigh
          ...
          : data.eeg.alphaRatio < 45
          ? `알파파가 낮아요. 자연 소리, 클래식 음악, 또는 5분 명상이 알파파 증가에 효과적이에요.`
          : `뇌파 상태가 좋아요. 현재 상태를 유지하세요.`
      }`,
      sources: ['EEG', '자율신경'],
      confidence: 'high',
    }
  ...
  if (hit(q, ['수면', '잠', '피곤', '피로', '불면', '잠이', '수면의', '졸려', '피곤해'])) {
    return {
      text: `수면 데이터: ${data.biosignal.sleepHours}시간 ${poorSleep ? '— 권장(7시간) 미만이에요' : '— 충분해요'}. HRV ${data.biosignal.hrv}ms로 ${lowHRV ? '수면 중 회복이 덜 이루어진 상태예요. 수면의 질이 낮을 가능성이 있어요' : '수면 회복이 잘 되고 있어요'}. EEG 스트레스 ${data.eeg.stressIndex}점이 ${stressHigh ? '높아서 입면에 방해가 될 수 있어요' : '높지 않아 수면 진입은 괜찮을 거예요'}. ${
        phase === 'luteal'
  ...
  if (hit(q, ['운동', '활동', '걷기', '달리기', '요가', '필라테스', '헬스', '근력', 'hiit', '어떤 운동', '운동 해도'])) {
    const recs: Record<CyclePhase, string> = {
      ...
      luteal: `황체기에는 에너지가 점점 감소해요. ${stressHigh ? `EEG 스트레스 ${data.eeg.stressIndex}점 — 고강도 운동이 오히려 코르티솔을 올릴 수 있으니 ` : ''}필라테스, 수영, 가벼운 걷기가 코르티솔 관리에 좋아요. 무리하면 PMS 증상이 심해질 수 있어요.`,
    }
    return {
      text: recs[phase],
      sources: ['주기', 'HRV', 'EEG'],
  ...
  if (hit(q, ['음식', '먹어', '영양', '보충', '비타민', '미네랄', '식단', '뭐 먹', '추천', '먹어야', '식품', '영양제'])) {
    const recs: Record<CyclePhase, string> = {
      ...
      luteal: `황체기 핵심 영양소: 마그네슘 300-400mg(PMS 핵심 완화제), 칼슘 1000mg(유제품·멸치), 비타민 B6 50mg. ${stressHigh ? `EEG 스트레스가 높으니 트립토판 식품(터키·달걀·오트밀)으로 세로토닌을 보충하세요. ` : ''}단당류·카페인은 증상 악화 원인이에요. 복합 탄수화물(고구마·현미)을 선택하세요.`,
    }
    return {
      text: recs[phase],
      sources: ['주기', 'EEG', '열화상'],
  ...
  if (hit(q, ['호르몬', '에스트로겐', '프로게스테론', '코르티솔', '호르몬 균형', '인슐린', 'lh', 'fsh'])) {
    const hMap: Record<CyclePhase, string> = {
      ...
      luteal: `프로게스테론 상승 구간이에요. ${stressHigh ? `EEG 스트레스 지수 ${data.eeg.stressIndex}점 — 코르티솔이 프로게스테론 합성 경로(프레그네놀론)를 '훔쳐'가는 '코르티솔 스틸' 현상이 일어날 수 있어요. 지금 스트레스 관리가 호르몬 균형의 핵심이에요. ` : '프로게스테론 생성 환경이 양호해요. '}마그네슘(300mg)과 아연(15mg)이 프로게스테론 합성을 직접 지원해요.`,
    }
    return {
      text: hMap[phase],
      sources: ['주기', 'EEG', '열화상'],
  ...
  if (hit(q, ['hrv', '심박', '심장', '맥박', '자율신경', '회복', '회복력', '심박수'])) {
    return {
      ...
      } EEG 자율신경 균형: 부교감 ${data.eeg.ansBalance}% — ${ansSym ? '교감 우세 상태예요. 심호흡·명상이 빠르게 부교감을 활성화해줘요.' : '균형 잡힌 상태예요.'}${phase === 'luteal' ? ' 황체기에는 체온 상승으로 HRV가 자연스럽게 낮아질 수 있으니 비교 시 참고하세요.' : ''}`,
      sources: ['바이오신호', 'EEG'],
      confidence: 'high',
    }
  }
  ...
  if (hit(q, ['피부', '여드름', '트러블', '피부 상태', '건조', '광택', '화장', '기미', '잡티'])) {
    ...
    return {
      text: `홍채 피부 Zone ${data.iris.skinZone}점 ${skinLow ? '— 피부 장벽 약화 신호가 있어요' : '— 피부 상태가 좋아요'}. ${
        phase === 'luteal'
          ? `황체기에는 프로게스테론이 피지 분비를 늘려 여드름이 생기기 쉬운 시기예요. 스트레스 지수 ${data.eeg.stressIndex}점 ${stressHigh ? '— 코르티솔이 피지 과분비를 더 자극하고 있어요. 스트레스 관리가 피부 관리예요.' : '— 스트레스가 높지 않아 다행이에요.'}  저자극 클렌징과 나이아신아마이드 성분을 써보세요.`
          : phase === 'follicular'
  ...
  if (hit(q, ['기분', '감정', '우울', '예민', '짜증', '감정 기복', 'pms', '감정적', '눈물', '불안해'])) {
    return {
      text: `감정 패턴 분석 (D+${cycleDay} ${phaseName}): EEG 스트레스 ${data.eeg.stressIndex}점, 알파파 ${data.eeg.alphaRatio}%(이완), 자율신경 균형 ${data.eeg.ansBalance}%. ${
        phase === 'luteal'
  ...
  if (hit(q, ['오늘', '현재', '지금', '컨디션', '상태', '전반', '종합', '어때', '어떤', '괜찮', '어떻게', '건강'])) {
    const score = Math.round(
      irisAvg * 0.18 +
      (100 - data.eeg.stressIndex) * 0.28 +
      data.eeg.ansBalance * 0.18 +
    ...
    return {
      text: `종합 건강 지수 ${score}점 (${level}). D+${cycleDay} ${phaseName} 기준 5가지 지표 분석: EEG 스트레스 ${data.eeg.stressIndex}점(${stressHigh ? '↑높음' : stressMid ? '보통' : '↓양호'}), 자궁 온도 ${data.thermal.uterineTemp}°C(${coldUterus ? '냉기' : '정상'}), HRV ${data.biosignal.hrv}ms(${lowHRV ? '낮음' : '정상'}), 홍채 ${irisAvg}점, 수면 ${data.biosignal.sleepHours}h(${poorSleep ? '부족' : '충분'}). ${
        score >= 72
          ? `전반적으로 좋은 컨디션이에요! ${phaseName}의 특성을 활용해 ${phase === 'follicular' || phase === 'ovulation' ? '운동과 활동을 늘려보세요' : '몸을 충분히 쉬어주세요'}.`
          : score >= 52
          ? `보통 상태예요. ${stressHigh ? 'EEG 스트레스 관리' : ''}${coldUterus ? (stressHigh ? '와 자궁 온열 케어' : '자궁 온열 케어') : ''}${poorSleep ? (stressHigh || coldUterus ? ', 수면 개선' : '수면 개선') : ''}이 지금 가장 중요한 과제예요.`
          : `컨디션이 낮아요. 오늘은 무리하지 말고 충분한 휴식, 따뜻한 식사, 수분 섭취에 집중해주세요. 내일은 더 나아질 거예요.`
      }`,
      sources: ['주기', 'EEG', '열화상', 'HRV', '홍채'],
      confidence: 'high',
    }
  }
  ...
  // ── DEFAULT ────────────────────────────────────────────────────
  return {
    text: `D+${cycleDay} ${phaseName} 기준으로 현재 주요 지표예요: EEG 스트레스 ${data.eeg.stressIndex}/100, 자궁 온도 ${data.thermal.uterineTemp}°C, HRV ${data.biosignal.hrv}ms, 홍채 점수 ${irisAvg}점, 수면 ${data.biosignal.sleepHours}시간. 더 구체적인 분석을 원하시면 아래 추천 질문 중 하나를 눌러보세요!`,
    sources: ['주기', 'EEG', '열화상', '바이오신호'],
    confidence: 'medium',
  }
}
