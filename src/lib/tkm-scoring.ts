...
export interface TemperamentInfo {
  ...
  nature: string
  traits: string[]
  strongPoints: string[]   // 강한 신체·기능
  weakOrgans: string[]     // 취약한 장부·부위
  weakness: string
  badFoods: string[]       // 피해야 할 음식
  badHabits: string[]      // 피해야 할 습관
  harmony: string[]
  foods: string[]
  color: string
  emoji: string
...
export const TEMPERAMENTS: Record<OrganKey, TemperamentInfo> = {
  heart: {
    ...
    strongPoints: ['혈액순환 · 체온 조절', '감정 표현력과 공감력', '빠른 상황 판단력', '면역 열 반응'],
    weakOrgans: ['심장 · 심혈관계', '소장 (영양 흡수)', '혀 · 언어 기능', '정신 안정 · 수면'],
    weakness: '과도한 열정으로 쉽게 소진될 수 있습니다. 스트레스가 쌓이면 심박수나 수면에 민감하게 반응해요.',
    badFoods: ['카페인 (커피·에너지드링크)', '알코올', '맵고 자극적인 음식', '고지방·고콜레스테롤 음식', '과도한 설탕·단 음식'],
    badHabits: ['밤샘 작업·늦은 취침', '감정 억압하기', '과도한 자극적 미디어', '흥분 상태로 잠들기', '식사 불규칙'],
    harmony: ['하루 5분 심호흡 명상', '자기 전 스마트폰 차단', '규칙적인 수면 패턴 유지', '감정을 말로 표현하기'],
  ...
  liver: {
    ...
    strongPoints: ['해독·정화 기능', '근육 힘과 유연성', '결단력과 실행력', '신진대사 조절'],
    weakOrgans: ['간 · 담낭', '눈 · 시신경', '근육 · 힘줄', '손발톱 · 관절'],
    weakness: '억눌린 분노나 스트레스가 근육 긴장·눈 피로로 나타납니다. 과로가 쌓이면 몸이 먼저 신호를 보내요.',
    badFoods: ['알코올 (간에 직접 타격)', '튀김·기름진 가공식품', '인스턴트·화학첨가물', '과도한 신맛 (식초·레몬 과다)', '늦은 밤 야식'],
    badHabits: ['야근·수면 부족 (간은 밤 11시~새벽 3시 회복)', '분노·짜증 억압', '장시간 스크린 응시', '과도한 음주·흡연', '운동 없이 앉아만 있기'],
    harmony: ['20분마다 눈 스트레칭', '저녁 야외 산책', '감정을 글·대화로 표현하기', '밤 11시 전 취침'],
  ...
  spleen: {
    ...
    strongPoints: ['소화 흡수 · 영양 운반', '면역 기반 유지', '사지 근육 에너지', '혈액 생성 기능'],
    weakOrgans: ['비장 · 위장 · 췌장', '입술 · 구강 점막', '사지 근육 전반', '림프 · 면역 계통'],
    weakness: '지나친 걱정과 반추가 소화기계에 부담을 줍니다. 차갑고 날것의 음식에 특히 민감해요.',
    badFoods: ['차가운 음료·아이스크림', '날것·생채소 과다 (냉해)', '과도한 단 음식·밀가루', '기름진 음식·튀김', '폭식·과식'],
    badHabits: ['불규칙한 식사 시간', '식후 바로 눕거나 격한 운동', '과도한 걱정·반추', '앉아만 있는 생활', '끼니 거르기'],
    harmony: ['규칙적인 식사 시간 지키기', '식후 10분 가벼운 산책', '따뜻한 음식 위주 식단', '걱정 일기로 생각 내려놓기'],
  ...
  lung: {
    ...
    strongPoints: ['호흡 · 산소 공급', '피부 · 체표 방어막', '면역 1차 방어', '체액 조절 기능'],
    weakOrgans: ['폐 · 기관지 · 코', '대장 · 피부', '체모 · 땀샘', '면역 점막 계통'],
    weakness: '슬픔이나 상실감을 억누르면 호흡이 얕아지고 피부가 건조해집니다. 환절기 면역 변화에 특히 민감해요.',
    badFoods: ['차갑고 날것의 음식', '유제품 과다 (담 생성)', '맵고 자극적인 음식 과다', '건조한 환경의 과자·인스턴트', '흡연·간접흡연'],
    badHabits: ['슬픔·감정 억압', '건조한 실내 환경 방치', '운동 부족·얕은 호흡', '환기 안 하기', '과도한 완벽주의 스트레스'],
    harmony: ['복식 호흡 10분 연습', '슬픔을 충분히 표현하기', '실내 가습·환기 관리', '피부 보습 루틴 지키기'],
  ...
  kidney: {
    ...
    strongPoints: ['생명력 · 선천 에너지 저장', '뼈 · 골수 · 치아 강도', '뇌 기능 · 기억력', '호르몬 · 생식 기능'],
    weakOrgans: ['신장 · 방광', '뼈 · 관절 · 허리', '귀 · 청력', '생식기 · 호르몬 계통'],
    weakness: '두려움과 만성 불안이 지속되면 호르몬 균형이 무너지고 관절에 영향을 줍니다. 몸이 쉽게 붓고 피로가 쌓여요.',
    badFoods: ['짠 음식·가공식품 (나트륨 과다)', '차갑고 생것 위주 식단', '카페인·알코올 과다', '인스턴트·방부제 음식', '과도한 단백질 (신장 부담)'],
    badHabits: ['만성 과로·야근', '수분 부족 (물 안 마시기)', '하체·발 냉기 방치', '두려움·불안 회피', '수면 부족'],
    harmony: ['하루 1.5L 이상 수분 섭취', '발·발목 따뜻하게 유지', '족욕으로 하체 순환 촉진', '밤 12시 전 취침'],
